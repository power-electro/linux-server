...
if [ ! -d ${OPENSHIFT_RUNTIME_DIR}/srv ]; then	
...
if [ ! -d ${OPENSHIFT_RUNTIME_DIR}/tmp ]; then	
...
if [ ! -d "$OPENSHIFT_RUNTIME_DIR/srv/pcre/bin" ]; then
	...
	if [ ! -d ${OPENSHIFT_RUNTIME_DIR}/srv/pcre ]; then	
		mkdir $OPENSHIFT_RUNTIME_DIR/srv/pcre
	fi
	...
	--prefix=$OPENSHIFT_RUNTIME_DIR/srv/pcre && make && make install && make clean " > $OPENSHIFT_LOG_DIR/pcre.log 2>&1 & 
	bash -i -c 'tail -f $OPENSHIFT_LOG_DIR/pcre.log'
	cd ..
fi
...
if [ ! -d "$OPENSHIFT_RUNTIME_DIR/srv/httpd/bin" ]; then
	...
	--enable-mime \
	--enable-deflate \
	--enable-headers && make && make install && make clean  " > $OPENSHIFT_LOG_DIR/pcre.log 2>&1 & 
	bash -i -c 'tail -f $OPENSHIFT_LOG_DIR/pcre.log'
	cd ..
fi
#echo "INSTALL ICU"
...
if [ ! -d "$OPENSHIFT_RUNTIME_DIR/srv/zlib" ]; then
	...
	nohup sh -c "./configure \
	--prefix=$OPENSHIFT_RUNTIME_DIR/srv/zlib &&  make && make install && make clean  " > $OPENSHIFT_LOG_DIR/Zlib_install.log 2>&1 & 
	bash -i -c 'tail -f $OPENSHIFT_LOG_DIR/Zlib_install.log'	
	cd ..
fi
...
if [ ! -d "$OPENSHIFT_RUNTIME_DIR/srv/php/etc/apache2" ]; then
	...
	#libiconv
	cd $LIBICONF
	nohup sh -c "./configure -q --enable-extra-encodings --prefix=$OPENSHIFT_RUNTIME_DIR/srv/php && make -s &&	make install " > $OPENSHIFT_LOG_DIR/php_install.log 2>&1 & 
	bash -i -c 'tail -f $OPENSHIFT_LOG_DIR/php_install.log'	

	#libxml2
	cd ../$LIBXML2
	nohup sh -c "./configure-q\
	--with-iconv=$OPENSHIFT_RUNTIME_DIR/srv/php \
	--prefix=$OPENSHIFT_RUNTIME_DIR/srv/php && 	make -s && 	make install " > $OPENSHIFT_LOG_DIR/libxml2_install.log 2>&1 & 
